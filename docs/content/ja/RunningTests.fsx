...
このセクションは FsCheck のテストを実行する様々な方法を説明します:
...
このガイドの前のセクションをお読みになられた方は、このメソッドはこれ以降、よく知っておくべきです。関数のように性質をシンプルに書くことができ、
`Check` 型のメソッドの1つとして呼び出すことができます。

`Check` のメソッドについて、基本的に2つのバージョンがあります: `Check.One` と `Check.All` です。

`Check.One` は1つの性質に対してテストを実行します。引数として `Config` 型のインスタンスを取りますが、これは FsCheck テストランナーがどのように動作するかの
多くの側面を設定できるようにし、他にはテストデータのサイズとランダムシードが使われます(ですので、実行を再生することができます)。 `Check` 上のほとんどのメソッドは
特定の設定でテストを実行するための略記です。例えば、 `Check.Quick` は `Check.One(Config.Quick, <property>)` と同等です。

`Config.Verbose` と `Check.Verbose` に注意しておきましょう。これらはテストごとの引数を印字し、例えばある入力でテストが無限ループした場合に便利です。

`Check.QuickThrowOnFailure` と `Check.VerboseThrowOnFailure` にも注意しておきましょう。これらは別のテストフレームワークのユニットテストに使われることを
意図しています。FsCheck ランナーを直接呼び出し、失敗した場合には(例外を)送出するように設定します。全てのフレームワークで、私はテストが失敗したというシグナルに
気づいているので、これらはあなたが既にお持ちのユニットテストを FsCheck のテストに統合する簡単な方法です。
...
デフォルトでは、FsCheck は`sprintf "%A"` 、すなわち構造化フォーマットを使用して生成された引数を印字します。これは通常あなたの期待通りのことをしてくれます、
つまり、プリミティブ型は値を、オブジェクトは ToString のオーバーライドなどです。もしそうでなければ(動機のあるケースは COM オブジェクトをテストすることです
