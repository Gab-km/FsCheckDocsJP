...
FsCheck はこのパターンも使っています、つまり `NonNegativeInt` 、 `PositiveInt` 、 `StringWithoutNullChars` などです。
`Arb.Default` 型にあるデフォルトの Arbitrary インスタンスを見てみましょう。
...
* あなたのプロジェクトにある fsx ファイルに性質やジェネレータを追加することで。実行するのは簡単で、ctrl-a を押してから alt-enter を押すだけで、結果が F# Interactive に表示されます。ソリューションに組み込まれている dll を参照するときは注意してください。 F# Interactive はセッションの残りの間はずっとそれらをロックし、セッションを終了するまでビルドすることが出来ません。1つの解決策は、dll の代わりにソースファイルを(ソリューションに)含めることですが、それは処理を遅くします。小規模なプロジェクトに有用です。デバッグするのは難しいですが。
* 別のコンソールアプリケーションを作成することで。デバッグは簡単ですし、アセンブリに迷惑なロックを行いません。テストのために FsCheck のみを使用し、性質が複数のアセンブリをまたぐような場合は、最良の選択肢です。
* 別のユニットテストフレームワークを使用することで。 FsCheck とユニットテストの手法が混在し(いくつかのものはユニットテストを使ってチェックした方が簡単ですし、逆もまた然りです)、グラフィカルランナーを好む場合に便利です。あなたが使用しているユニットテストフレームワーク次第では、無料で Visual Studio と上手く統合できるでしょう。このシナリオで FsCheck をカスタマイズする方法は、上記を参照してください。
 *)
