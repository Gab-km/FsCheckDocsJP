...
テストデータのジェネレータとシュリンカ(詳しくは [テストデータ](TestData_ja.html))をカプセル化します。
...
ジェネレータを定義するためのコンビネータを [テストデータ](TestData_ja.html) で説明します。
...
* `<property> .|. <property>` は性質のどちらかが成功した場合に成功し、両方の性質が失敗した場合に失敗し、両方共棄却された場合に棄却されます。
